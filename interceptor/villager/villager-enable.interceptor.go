...
func CheckEnableVillagerValueInterceptor(c *gin.Context) {
	var request villager_model.VillageEnableRequest
	...
	if err != nil {
		//--------create error log
		format_utls.WriteLog(format_utls.GetErrorLogVillagerEnableFile(), fmt.Sprintf("Error parsing JSON string - %s", err))
		fmt.Printf("Error parsing JSON string - %s", err)
	}

	errValidation := checkSlotEnableRequest(request)
	if errValidation != nil {
		...
		if err != nil {
			format_utls.WriteLogInterface(format_utls.GetErrorLogVillagerEnableFile(), nil, constants.MessageCovertObjTOJSONFailed)
...
func checkSlotEnableRequest(request villager_model.VillageEnableRequest)  error {
